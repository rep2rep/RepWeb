...
module Make: S = (
  ...
  module Template = {
    ...
    let make = (
      ...
      if path->Array.some(v => closedFolders->Array.includes(v)) {
        ...
        switch fileOrFolder {
        | FileTree.FileOrFolder.File((id, datum)) => {
            let name = datum->D.name
            let props = {
              "id": id,
  ...
  let make = (
    ...
    let isData =
      active
      ->Option.map(active =>
        data->FileTree.getFolderPathAndPosition(active)->Option.isSome->Bool.not
      )
    ...
    let selectedPath =
      ...
      ->Option.flatMap(id =>
        if isData {
          data->FileTree.getFilePathAndPosition(((id', _)) => id === id')->Option.map(fst)
        } else {
          data
    ...
    <HideablePanel
      ...
      <h1 style={ReactDOM.Style.make(~padding="1rem", ())}> {React.string(title)} </h1>
      <div
        style={ReactDOM.Style.make(
          ~fontSize="0.7rem",
          ~fontWeight="bold",
        ...
        <div
          ...
          onDrop={e => {
            ...
            let (keep, reject) = files->Array.partition(f => {
              ...
              importExtensions->Array.some(extn => fname->String.endsWith(extn))
            })
            if keep != [] {
            ...
            if reject != [] {
              Dialog.alert(
                ...
                reject->Array.map(f => "  " ++ File.name(f) ++ "\n")->Js.Array2.joinWith("") ++
                "Not " ++
                Array.joinWith(importExtensions, ", ") ++ " files.",
              )
            }
          }}
        />
      ...
      <div
        ...
        <div
          ...
          <Button
            onClick={_ => active->Option.iter(onDuplicate)} enabled={isData} value="Duplicate"
          />
        ...
        <div
          ...
          <Button onClick={_ => active->Option.iter(onExport)} enabled={isData} value="Export" />
          <input
            name="import_data"
            id="import_data"
            type_="file"
            accept={importExtensions->Array.joinWith(",")}
            multiple={true}
            style={ReactDOM.Style.make(
              ~width="0.1px",
              ~height="0.1px",
              ...
              active->Option.iter(active => {
                paths->Array.forEach(((_, f)) => {
                  ...
                  | FileTree.FileOrFolder.File((id, datum)) =>
                    if id === active {
                      let name = datum->D.name
                      if Dialog.confirm("Definitely delete '" ++ name ++ "'?") {
                        onDelete(id)
                      }
                    }
                  | FileTree.FileOrFolder.Folder(name, id) =>
                    if id === active {
                      let contents = data->FileTree.folderContents(id)
                      if contents->Option.map(FileTree.isEmpty)->Option.getWithDefault(false) {
                        onDeleteFolder(id)
                        closedFolders->Array.filter(v => v !== id)->setClosedFolders
                      } else if (
                        Dialog.confirm(
                          "Delete the folder '" ++ name ++ "', along with all the data inside it?",
                        )
                      ) {
  ...
  let make = React.memoCustomCompareProps(make, (old_, new_) => {
    let old_data = FileTree.asFlat(old_["data"])
    let new_data = FileTree.asFlat(new_["data"])
    old_["importExtensions"] == new_["importExtensions"] &&
    old_["id"] === new_["id"] &&
    old_["active"] === new_["active"] &&
    old_["title"] === new_["title"] &&
    old_["version"] === new_["version"] &&
    Array.length(old_data) === Array.length(new_data) &&
    Array.zip(old_data, new_data)->Array.every(((i, j)) =>
      switch (i, j) {
      | (FileTree.FileOrFolder.File((i, im)), FileTree.FileOrFolder.File((j, jm))) =>
        i === j && D.name(im) === D.name(jm)
